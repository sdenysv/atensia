...
private let kSourceKey  = "widget_source"  // "app" | "widget"
...
struct SetMoodIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        defaults?.set("widget",      forKey: kSourceKey)
...
struct MoodProvider: TimelineProvider {
    ...
    private func readEntry() -> MoodEntry {
        ...
        if defaults?.string(forKey: kDateKey) != today {
            ...
            defaults?.set("app",   forKey: kSourceKey)
        }
        return MoodEntry(
            date:    Date(),
                ...
                ForEach(0..<3, id: \.self) { i in
                    Button(intent: SetMoodIntent(key: key, value: values[i])) {
                        Text(options[i])
                            ...
                            .frame(minHeight: 30)
                            .background(selectedIndex == i ? Color.primary : Color.clear)
                    }
                    .buttonStyle(.plain)
